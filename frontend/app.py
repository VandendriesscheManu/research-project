...
# Field options organized by step
step_field_options = {
...
# Get field options for current step
current_field_options = step_field_options.get(st.session_state.current_step, {})
...
if current_field_options:
    ...
    with col1:
        selected_field = st.selectbox(
            ...
            options=list(current_field_options.keys()),
            format_func=lambda x: current_field_options[x],
            key=f"field_selector_step_{st.session_state.current_step}"
        )
    with col2:
        st.write("")  # Spacing
        st.write("")  # Spacing
    ...
    if suggest_button:
        if not st.session_state.api_key:
            ...
            try:
                with st.spinner(f"✨ AI is generating suggestion for {current_field_options[selected_field]}..."):
    ...
    st.info("💡 No AI suggestions available for this step's fields.")
