    ...
    try:
        with st.spinner(f"✨ Generating suggestion..."):
            ...
            if response.status_code == 200:
                ...
                # Extract content without provider tag
                content = suggestion
                ...
                st.success(f"✅ {field_label} updated!")
                st.rerun()
            else:
...
if st.session_state.current_step == 1:
    ...
    with col1:
        ...
        col_cat, col_cat_btn = st.columns([9, 1])
        with col_cat:
    ...
    with col2:
        col_feat, col_feat_btn = st.columns([9, 1])
        with col_feat:
        ...
        col_usp, col_usp_btn = st.columns([9, 1])
        with col_usp:
    ...
    col_brand, col_brand_btn = st.columns([9, 1])
    with col_brand:
